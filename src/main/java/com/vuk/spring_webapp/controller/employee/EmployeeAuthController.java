...
@RestController
@RequestMapping("${api.prefix}/employees/auth")
public class EmployeeAuthController {
    ...
    public ResponseEntity<ApiResponse> authenticateCandidate(@RequestBody LoginRequest request) {
        try {
            var response = userService.authenticateUser(request.getEmail(), request.getPassword(), Role.EMPLOYEE);
    ...
    public ResponseEntity<ApiResponse> registerCandidate(@RequestBody RegisterEmployeeRequest request) {
        try {
